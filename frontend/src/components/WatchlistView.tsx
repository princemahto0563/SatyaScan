...
export function WatchlistView() {
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/60 p-5 shadow-sm flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <div className="flex items-center space-x-2">
            <h1 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white tracking-tight">
              Prototype Reference Watchlist
            </h1>
            <span className="rounded bg-amber-100 dark:bg-amber-950/60 px-2 py-0.5 text-[10px] font-semibold text-amber-800 dark:text-amber-400 border border-amber-300 dark:border-amber-700/60">
              SYNTHETIC REFERENCE DATA
            </span>
          </div>
          <p className="mt-1 text-xs text-slate-600 dark:text-slate-400">
            Simulates border watchlists (stolen passports, lookout circulars) without accessing classified government databases.
          </p>
        </div>
        ...
        <div className="relative">
          ...
          <input
            ...
            className="rounded-lg border border-slate-300 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/80 pl-9 pr-4 py-1.5 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-teal-600 w-full sm:w-64"
          />
        </div>
      </div>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
              {filtered.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition">
                  <td className="px-5 py-3.5 font-mono font-bold text-teal-700 dark:text-teal-300">
                    {item.document_id}
                  </td>
                  ...
                  <td className="px-4 py-3.5">
                    <span className="rounded bg-rose-100 dark:bg-rose-950/60 px-2 py-0.5 text-[10px] font-semibold text-rose-800 dark:text-rose-400 border border-rose-300 dark:border-rose-700/60">
                      {item.risk_category}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3.5">
                    <span className="text-emerald-700 dark:text-emerald-400 font-medium text-[11px] flex items-center space-x-1">
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-5 py-8 text-center text-slate-500">
                    No watchlist entries found matching your search.
                  </td>
                </tr>
              )}
            </tbody>
